...
namespace DeltaBotFour.Infrastructure.Implementation
{
    public class CommentValidator : ICommentValidator
    {
        ...
        {
            // Cannot award users if a post is flaired with specified values in appsettings.json
            var acceptableFlairs = _appConfiguration.NoSnipeFlairs;
            if (acceptableFlairs != null && acceptableFlairs.Any())
            {
                return _commentBuilder.BuildReply(DB4CommentType.FailSnipeNotAllowed, comment);
            }
