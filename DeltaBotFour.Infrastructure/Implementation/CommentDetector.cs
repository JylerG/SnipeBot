...
namespace DeltaBotFour.Infrastructure.Implementation
{
    public class CommentDetector : ICommentDetector
    {
        ...
        // Constructor now checks for null values to avoid NullReferenceException
        public CommentDetector(AppConfiguration appConfiguration)
        {
            if (appConfiguration?.Comments?.AllComments == null)
            {
                throw new InvalidOperationException("Comments configuration is not initialized.");
            }

            if (appConfiguration?.ReplaceTokens?.AllTokens == null)
            {
                throw new InvalidOperationException("ReplaceTokens configuration is not initialized.");
            }
